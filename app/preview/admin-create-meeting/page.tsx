"use client";

import { useState } from "react";
import { PreviewShell } from "../PreviewShell";

// ── Dropdown menu ──────────────────────────────────────────────────────────

function MeetingDropdown({ open, onClose }: { open: boolean; onClose: () => void }) {
  if (!open) return null;
  return (
    <div className="absolute left-0 top-full z-10 mt-1 w-64 rounded-lg bg-white shadow-sm dark:bg-gray-700">
      <ul className="p-2 text-sm font-medium text-gray-500 dark:text-gray-400">
        <li>
          <a
            href="#"
            onClick={onClose}
            className="inline-flex w-full items-center rounded-md px-3 py-2 hover:bg-gray-100 hover:text-gray-900 dark:hover:bg-gray-600 dark:hover:text-white"
          >
            <svg className="me-1.5 h-5 w-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
              <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 12h14m-7 7V5" />
            </svg>
            Start an instant meeting
          </a>
        </li>
        <li>
          <a
            href="#"
            onClick={onClose}
            className="inline-flex w-full items-center rounded-md px-3 py-2 hover:bg-gray-100 hover:text-gray-900 dark:hover:bg-gray-600 dark:hover:text-white"
          >
            <svg className="me-1.5 h-5 w-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
              <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
            </svg>
            Create a meeting for later
          </a>
        </li>
        <li>
          <a
            href="#"
            onClick={onClose}
            className="inline-flex w-full items-center rounded-md px-3 py-2 hover:bg-gray-100 hover:text-gray-900 dark:hover:bg-gray-600 dark:hover:text-white"
          >
            <svg className="me-1.5 h-5 w-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
              <path
                stroke="currentColor"
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth="2"
                d="M4 10h16m-8-3V4M7 7V4m10 3V4M5 20h14a1 1 0 0 0 1-1V7a1 1 0 0 0-1-1H5a1 1 0 0 0-1 1v12a1 1 0 0 0 1 1Zm3-7h.01v.01H8V13Zm4 0h.01v.01H12V13Zm4 0h.01v.01H16V13Zm-8 4h.01v.01H8V17Zm4 0h.01v.01H12V17Zm4 0h.01v.01H16V17Z"
              />
            </svg>
            Schedule for later
          </a>
        </li>
      </ul>
    </div>
  );
}

// ── Page ───────────────────────────────────────────────────────────────────

export default function AdminCreateMeetingPage() {
  const [dropdownOpen, setDropdownOpen] = useState(false);

  return (
    <PreviewShell variant="admin" title="Create Meeting">
      <section>
        <div className="mx-auto grid h-[calc(100vh-8rem)] max-w-7xl place-items-center gap-16 px-4 md:grid-cols-12 xl:px-0">

          {/* Left: form */}
          <div className="col-span-full md:col-span-9 lg:col-span-6">

            {/* Brand */}
            <a href="#" className="mb-6 flex items-center text-2xl font-semibold text-gray-900 dark:text-white">
              <svg className="mr-2 h-8 w-8 text-[#1A56DB]" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
                <path fillRule="evenodd" d="M14.516 6.743c-.41-.606-1.053-.999-1.796-.999-1.226 0-2.213.998-2.22 2.221v.003A2.217 2.217 0 0 0 12.718 10.2c.745 0 1.4-.39 1.79-.98l.99 1.495a4.17 4.17 0 0 1-3.497 1.495 4.213 4.213 0 0 1-4.2-4.2 4.213 4.213 0 0 1 4.2-4.2c1.297 0 2.454.59 3.237 1.52l-.722 1.413ZM4 2h16a2 2 0 0 1 2 2v16a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2Zm14 9a6 6 0 1 1-12 0 6 6 0 0 1 12 0Z" clipRule="evenodd"/>
              </svg>
              Scripps Connect
            </a>

            {/* Headline */}
            <h1 className="mb-4 text-3xl font-bold leading-none tracking-tight text-gray-900 dark:text-white md:text-5xl">
              Seamless video meetings anytime, anywhere
            </h1>
            <p className="mb-4 text-lg text-gray-500 dark:text-gray-400 md:mb-6">
              Connect with colleagues, clients, or friends through a platform designed to make virtual meetings effortless.
            </p>

            {/* Action row */}
            <div className="mb-4 w-full items-center space-y-4 border-b border-gray-200 pb-4 dark:border-gray-800 sm:flex sm:space-x-4 sm:space-y-0 md:mb-6 md:pb-6">

              {/* New meeting button + dropdown */}
              <div className="relative shrink-0 w-full sm:w-auto">
                <button
                  type="button"
                  onClick={() => setDropdownOpen((v) => !v)}
                  className="inline-flex w-full items-center justify-center rounded-lg border border-[#1A56DB] bg-[#1A56DB] px-5 py-3 text-center text-sm font-medium text-white hover:border-blue-800 hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:border-blue-600 dark:bg-blue-600 dark:hover:border-blue-700 dark:hover:bg-blue-700 dark:focus:ring-blue-800 sm:w-auto"
                >
                  New meeting
                  <svg className="-me-0.5 ms-2 h-4 w-4" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                    <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="m19 9-7 7-7-7" />
                  </svg>
                </button>
                <MeetingDropdown open={dropdownOpen} onClose={() => setDropdownOpen(false)} />
              </div>

              {/* Code input */}
              <div className="w-full">
                <label htmlFor="meeting-code" className="sr-only">Meeting code</label>
                <div className="relative max-w-full">
                  <div className="pointer-events-none absolute inset-y-0 start-0 flex items-center ps-3.5">
                    <svg className="h-4 w-4 text-gray-500 dark:text-gray-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
                      <path
                        fillRule="evenodd"
                        d="M2 7a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V7Zm5.01 1H5v2.01h2.01V8Zm3 0H8v2.01h2.01V8Zm3 0H11v2.01h2.01V8Zm3 0H14v2.01h2.01V8Zm3 0H17v2.01h2.01V8Zm-12 3H5v2.01h2.01V11Zm3 0H8v2.01h2.01V11Zm3 0H11v2.01h2.01V11Zm3 0H14v2.01h2.01V11Zm3 0H17v2.01h2.01V11Zm-12 3H5v2.01h2.01V14ZM8 14l-.001 2 8.011.01V14H8Zm11.01 0H17v2.01h2.01V14Z"
                        clipRule="evenodd"
                      />
                    </svg>
                  </div>
                  <input
                    type="text"
                    id="meeting-code"
                    className="w-full rounded-lg border border-gray-200 bg-white p-3.5 ps-10 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder:text-gray-400 dark:focus:border-blue-500 dark:focus:ring-blue-500"
                    placeholder="Enter a code or link"
                  />
                </div>
              </div>

              {/* Join button */}
              <button
                type="button"
                className="w-full shrink-0 rounded-lg border border-gray-200 bg-white px-5 py-3 text-sm font-medium text-gray-900 hover:bg-gray-100 hover:text-[#1A56DB] focus:z-10 focus:outline-none focus:ring-4 focus:ring-gray-100 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white dark:focus:ring-gray-700 sm:w-auto"
              >
                Join
              </button>
            </div>

            {/* Trust badges */}
            <div className="flex items-center divide-x divide-gray-200 text-gray-500 dark:divide-gray-800 dark:text-gray-400">
              <div className="flex items-center pr-5 text-sm">
                <svg className="me-1 h-4 w-4" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
                  <path fillRule="evenodd" d="M4 5a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2H4Zm0 6h16v6H4v-6Z" clipRule="evenodd" />
                  <path fillRule="evenodd" d="M5 14a1 1 0 0 1 1-1h2a1 1 0 1 1 0 2H6a1 1 0 0 1-1-1Zm5 0a1 1 0 0 1 1-1h5a1 1 0 1 1 0 2h-5a1 1 0 0 1-1-1Z" clipRule="evenodd" />
                </svg>
                No credit card required
              </div>
              <div className="flex items-center pl-5 text-sm">
                <svg className="me-1 h-4 w-4" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
                  <path fillRule="evenodd" d="M8 10V7a4 4 0 1 1 8 0v3h1a2 2 0 0 1 2 2v7a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2v-7a2 2 0 0 1 2-2h1Zm2-3a2 2 0 1 1 4 0v3h-4V7Zm2 6a1 1 0 0 1 1 1v3a1 1 0 1 1-2 0v-3a1 1 0 0 1 1-1Z" clipRule="evenodd" />
                </svg>
                GDPR ready
              </div>
            </div>

          </div>

          {/* Right: illustration (lg+ only) */}
          <div className="col-span-6 hidden lg:flex lg:items-center lg:justify-center">
            <div className="flex h-80 w-80 items-center justify-center rounded-2xl bg-blue-50 dark:bg-gray-800">
              <svg className="h-48 w-48 text-[#1A56DB] opacity-20 dark:opacity-10" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
                <path fillRule="evenodd" d="M14.516 6.743c-.41-.606-1.053-.999-1.796-.999-1.226 0-2.213.998-2.22 2.221v.003A2.217 2.217 0 0 0 12.718 10.2c.745 0 1.4-.39 1.79-.98l.99 1.495a4.17 4.17 0 0 1-3.497 1.495 4.213 4.213 0 0 1-4.2-4.2 4.213 4.213 0 0 1 4.2-4.2c1.297 0 2.454.59 3.237 1.52l-.722 1.413ZM4 2h16a2 2 0 0 1 2 2v16a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2Zm14 9a6 6 0 1 1-12 0 6 6 0 0 1 12 0Z" clipRule="evenodd"/>
              </svg>
            </div>
          </div>

        </div>
      </section>
    </PreviewShell>
  );
}
