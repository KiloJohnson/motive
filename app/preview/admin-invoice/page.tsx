...
import { Badge, Button, Dropdown, DropdownItem, DropdownDivider, Select } from "flowbite-react";
import { HiArrowLeft, HiPencilAlt, HiDownload, HiPrinter, HiCheck, HiClock, HiX,
         HiUser, HiCalendar, HiCreditCard, HiCurrencyDollar } from "react-icons/hi";
...
const details = [
  { icon: HiUser,            label: "Created by:",   value: "Jese Leos"        },
  { icon: HiCalendar,        label: "Due date:",      value: "08 July 2025"     },
  { icon: HiCreditCard,      label: "",               value: "Pay by Bank Transfer" },
  { icon: HiCurrencyDollar,  label: "Currency:",      value: "American Dollar"  },
];
    ...
    <PreviewShell variant="admin" title="Invoice">
      <div className="p-6 space-y-5">
        ...
        <nav className="text-sm text-gray-500 dark:text-gray-400 flex items-center gap-1.5">
          ...
          <span>/</span>
          <span className="text-gray-900 dark:text-white font-medium">Invoice #184325</span>
        </nav>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button size="sm" color="alternative">
              <HiPencilAlt className="mr-1.5 h-3.5 w-3.5" />Edit
            </Button>
            <Dropdown label="" renderTrigger={() => (
              <Button size="sm" color="alternative">
                <HiDownload className="mr-1.5 h-3.5 w-3.5" />Download <span className="ml-1">▾</span>
              </Button>
            )}>
              <DropdownItem>Save as PDF</DropdownItem>
              <DropdownItem>Save as Excel</DropdownItem>
              <DropdownItem>Save as Word</DropdownItem>
              <DropdownDivider />
              <DropdownItem icon={HiPrinter}>Print</DropdownItem>
            </Dropdown>
            <Button size="sm">
              <HiPrinter className="mr-1.5 h-3.5 w-3.5" />Print
            </Button>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-8 space-y-7">
          ...
          {/* Pay to / Invoice to */}
          <div className="grid grid-cols-2 gap-8 pt-2 border-t border-gray-100 dark:border-gray-700">
            <div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-6 flex flex-col gap-5">
          ...
          <div className="space-y-2.5 text-sm">
            {details.map((d, i) => (
              <div key={i} className="flex items-center gap-2 text-gray-600 dark:text-gray-400">
                <d.icon className="h-4 w-4 shrink-0 text-gray-400 dark:text-gray-500" />
                {d.label && <span className="font-medium text-gray-900 dark:text-white">{d.label}</span>}
                <span>{d.value}</span>
              </div>
            ))}
          </div>
          ...
          <div className="mt-auto flex items-start justify-between bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4">
            ...
            <button className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 ml-4 shrink-0">
